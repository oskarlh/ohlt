...
// These should match https://github.com/ValveSoftware/halflife/blob/master/engine/studio.h
struct studiohdr_t
{
	std::int32_t ident;
	std::int32_t version;
	...
	std::int32_t numtransitions;	// animation node to animation node transition graph
...
typedef struct 
{
	...
	std::int32_t flags; // ??
	std::array<std::int32_t, 6> bonecontroller; // bone controller index, -1 == none
...
typedef struct
{
	...
	std::int32_t animindex;	// mstudioanim_t pointer relative to start of sequence group data
