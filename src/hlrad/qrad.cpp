	...
	// This function has been rewritten because the original one is not totally correct and may fail to do what it claims.
{
	...
	// cut the winding by the direction of plane A and save into windingArray
	{
		...
		for (int i = 1; i < gridsizeA; i++)
		{
			...
			if (!front || front.WindingOnPlaneSide (plA->normal, dist, epsilon) == face_side::on) // ended
			{
				break;
			}
			if (!back || back.WindingOnPlaneSide (plA->normal, dist, epsilon) == face_side::on) // didn't begin
			{
				continue;
			}
	...
	// cut by the direction of plane B
	{
		...
		for (int i = 0; i < numstrips; i++)
		{
			...
			for (int j = 1; j < gridsizeB; j++)
			{
				...
				if (!front || front.WindingOnPlaneSide (plB->normal, dist, epsilon) == face_side::on) // ended
				{
					break;
				}
				if (!back || back.WindingOnPlaneSide (plB->normal, dist, epsilon) == face_side::on) // didn't begin
				{
					continue;
				}
...
static void     Settings()
{
	...
	snprintf (buf1, sizeof(buf1), "%zu (%s)", (std::size_t) g_transfer_compress_type, (const char*) float_type_string[(std::size_t) g_transfer_compress_type].data());
	snprintf (buf2, sizeof(buf2), "%zu (%s)", (std::size_t) cli_option_defaults::transferCompressType, (const char*) float_type_string[(std::size_t) cli_option_defaults::transferCompressType].data());
	Log("size of transfer     [ %17s ] [ %17s ]\n", buf1, buf2);
	snprintf (buf1, sizeof(buf1), "%zu (%s)", (std::size_t) g_rgbtransfer_compress_type, vector_type_string[(std::size_t) g_rgbtransfer_compress_type]);
