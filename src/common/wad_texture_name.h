...
// Not supported by Apple Clang in XCode 16.2, but should be supported by Clang 17
		...
		constexpr bool is_any_bevel() const noexcept {
			return is_ordinary_bevel() || is_bevelbrush() || is_bevel_hint() || is_any_clip_bevel();
		}
		...
		constexpr bool is_noclip() const noexcept {
			return equals_constant(u8"nullnoclip") || equals_constant(u8"noclip");
		}
		...
		constexpr bool is_clip_hull() const noexcept {
			return starts_with_constant(u8"cliphull") && units[8 + 1] >= u'0' &&  units[8 + 1] <= (u'0' + NUM_HULLS_X) && units[8 + 2] == u8'\0';
		}
		constexpr std::optional<std::uint8_t> get_clip_hull_number() const noexcept {
			if(is_clip_hull()) {
