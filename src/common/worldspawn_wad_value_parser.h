...
class worldspawn_wad_value_iterator {
    ...
    public:
		...
		constexpr worldspawn_wad_value_iterator& operator++() noexcept {
			do {
				...
				if(nextSeparator == std::u8string_view::npos) {
					...
					currentWad = filename_in_file_path_string(remaining.substr(0, nextSeparator));
		...
		constexpr worldspawn_wad_value_iterator(const worldspawn_wad_value_iterator&) = default;
        constexpr worldspawn_wad_value_iterator(std::u8string_view wadValue) noexcept : remaining(wadValue) {
			++*this;
		}

		constexpr worldspawn_wad_value_iterator& operator=(const worldspawn_wad_value_iterator&) = default;

    	constexpr std::u8string_view operator*() const noexcept {
			return currentWad;
		}

		constexpr bool operator!=(const worldspawn_wad_value_iterator& other) const noexcept {
			return remaining.length() != other.remaining.length() || currentWad.length() != other.currentWad.length();
		}
};
...
class worldspawn_wad_value_parser {
	...
	public:
		constexpr worldspawn_wad_value_parser(std::u8string_view wadString) : wadValue(wadString) {};
