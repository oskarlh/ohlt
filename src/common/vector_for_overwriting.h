...
template<class T> class vector_for_overwriting {
	static_assert(
	...
	public:
		friend inline void swap(vector_for_overwriting& a, vector_for_overwriting& b) {
			using std::swap;
		...
		constexpr vector_for_overwriting(const vector_for_overwriting& other) = default;
		constexpr vector_for_overwriting(vector_for_overwriting&& other) noexcept {
			swap(*this, other);
		}
		constexpr ~vector_for_overwriting() = default;


		constexpr vector_for_overwriting& operator=(const vector_for_overwriting& other) = default;
		constexpr vector_for_overwriting& operator=(vector_for_overwriting&& other) noexcept {
			swap(*this, other);
			return *this;
		}
		...
		constexpr void reset(std::size_t newRequestedSize) {
			if(newRequestedSize > allocated) {
				dataPtr.reset();
				// Since this container is meant for temporary data that will soon
				// be deallocated, we can afford to splurge and avoid having to do
				// too many allocations.
				allocated = std::max(newRequestedSize, newRequestedSize * 2 + 32);
