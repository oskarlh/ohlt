...
static bool TEX_InitFromWad(std::filesystem::path const & bspPath) {
		 ...
		 ++i) // loop through all wadpaths in map
	{
		...
		for (WadInclude_i it = g_WadInclude.begin();
			 it != g_WadInclude.end(); it++) // Check -wadinclude list
		{
	...
	for (std::size_t i = 0; i < g_pWadPaths.size(); ++i) {
		...
		if (!texfiles[nTexFiles] && pszWadroot) {
			...
			safe_snprintf(
				szTmp, _MAX_PATH, "%s" SYSTEM_SLASH_STR "%s%s", pszWadroot,
				szSubdir, szFile
			);
		...
		if (!texfiles[nTexFiles]) {
			...
			Fatal(
				assume_COULD_NOT_FIND_WAD, "Could not open wad file %s",
			...
			if (lumpinfo[nTexLumps].lump_info.disksize > MAX_TEXTURE_SIZE) {
				texturesOversized.push_back(std::make_tuple(
					(char const *) lumpinfo[nTexLumps]
						...
						.data(),
					szWadFileName, lumpinfo[nTexLumps].lump_info.disksize
				));
			}
		}
	...
	qsort(
		(void*) lumpinfo, (size_t) nTexLumps, sizeof(lumpinfo[0]),
		lump_sorter_by_name
	);
...
FindTexture(lumpinfo_with_wadfileindex const * const source) {
	...
	found = (lumpinfo_with_wadfileindex*) bsearch(
		source, (void*) lumpinfo, (size_t) nTexLumps, sizeof(lumpinfo[0]),
		lump_sorter_by_name
	);
	if (!found) {
		Warning(
...
static int LoadLump(
	...
	*texsize = 0;
	if (source->lump_info.filepos) {
		if (fseek(
				texfiles[source->iTexFile], source->lump_info.filepos,
				SEEK_SET
			)) {
		...
		*texsize = source->lump_info.disksize;
		...
		if (texwadpathes[source->iTexFile]->usedbymap) {
			...
			SafeRead(
				texfiles[source->iTexFile], writewad_data,
			...
			Developer(
				developer_level::message, "Including texture %s\n",
				source->lump_info.name.c_str()
			);
...
void WriteMiptex(std::filesystem::path const & bspPath) {
	...
	start = I_FloatTime();
	{
		...
		qsort(
			(void*) miptex, (size_t) nummiptex, sizeof(miptex[0]),
	...
	start = I_FloatTime();
	{
										...
										// data to the temp wad file
		{
			...
			int writewad_datasize;
			len = LoadLump(
				miptex + i, data, &texsize,
				&g_dtexdata[g_max_map_miptex] - data, writewad_data,
				writewad_datasize
			); // Load lump data
		...
		SafeWrite(
			writewad_file, writewad_lumpinfos,
...
void LogWadUsage(wadpath_t* currentwad, int nummiptex) {
	...
	Log("[%s] %i/%i texture%s (%2.2f%%)\n - %s\n", currentwadName,
		currentwad->usedtextures, currentwad->totaltextures,
		currentwad->usedtextures == 1 ? "" : "s", percentUsed,
